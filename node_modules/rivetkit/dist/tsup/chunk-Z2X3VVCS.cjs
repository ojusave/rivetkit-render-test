...
var _cborx = require('cbor-x'); var cbor = _interopRequireWildcard(_cborx); var cbor2 = _interopRequireWildcard(_cborx); var cbor3 = _interopRequireWildcard(_cborx); var cbor4 = _interopRequireWildcard(_cborx); var cbor5 = _interopRequireWildcard(_cborx); var cbor6 = _interopRequireWildcard(_cborx);
...
var ActorInspector = (_class = class {
  ...
  getWorkflowHistory() {
    ...
    return _nullishCoalesce(history, () => ( null));
  }
  ...
  async getDatabaseSchema() {
    ...
    if (!this.isDatabaseEnabled()) {
      throw new (0, _chunkL47L3ZWJcjs.DatabaseNotEnabled)();
    }
...
var ConnectionManager = class {
  ...
  async cleanupPersistedHibernatableConnections(reason) {
    ...
    this.#actor.rLog.info({
      ...
      reason: _nullishCoalesce(reason, () => ( "unknown")),
...
var QueueManager = class {
  ...
  async enqueue(name, body) {
    ...
    if (encodedSize > this.#actor.config.options.maxQueueMessageSize) {
      throw new (0, _chunkL47L3ZWJcjs.QueueMessageTooLarge)(
        encodedSize,
  ...
  async receive(names, count, timeout, abortSignal, completable = false) {
    ...
    const onAbort = () => {
      rejectWaiter(new (0, _chunkL47L3ZWJcjs.ActorAborted)());
    };
    const onStop = () => {
      rejectWaiter(new (0, _chunkL47L3ZWJcjs.ActorAborted)());
    };
  ...
  async waitForNames(names, abortSignal) {
    ...
    return await new Promise((resolve, reject) => {
      ...
      const actorAbortSignal = this.#actor.abortSignal;
      const onActorAbort = () => listener.reject(new (0, _chunkL47L3ZWJcjs.ActorAborted)());
      if (actorAbortSignal.aborted) {
        onActorAbort();
        return;
      }
      ...
      if (abortSignal) {
        const onAbort = () => listener.reject(new (0, _chunkL47L3ZWJcjs.ActorAborted)());
        if (abortSignal.aborted) {
          onAbort();
          return;
        }
  ...
  initPersistProxy(target) {
    ...
    if (target === null || typeof target !== "object") {
      ...
      if (!_chunk2Q5WS6QHcjs.isCborSerializable.call(void 0, 
        ...
        throw new (0, _chunkL47L3ZWJcjs.InvalidStateType)({ path: invalidPath });
      }
      return target;
    }
    if (this.#persist) {
        ...
        await this.#persistWriteQueue.enqueue(async () => {
          ...
          for (const connId of this.#actor.connectionManager.connsWithPersistChanged) {
            ...
            this.#actor.rLog.info({
              ...
              connId,
              gatewayId: _enginerunner.idToStr.call(void 0, hibernatableDataRaw.gatewayId),
...
var ActorInstance = (_class2 = class {
  ...
  startTraceSpan(name, attributes) {
    return this.#traces.startSpan(name, {
      parent: _nullishCoalesce(this.#traces.getCurrentSpan(), () => ( void 0)),
  ...
  emitTraceEvent(name, attributes, handle) {
    const span = _nullishCoalesce(handle, () => ( this.#traces.getCurrentSpan()));
    if (!span) {
      return;
    }
  ...
  assertReady(allowStoppingState = false) {
    if (!this.#ready) throw new (0, _chunkL47L3ZWJcjs.InternalError)("Actor not ready");
    if (!allowStoppingState && this.#stopCalled)
      throw new (0, _chunkL47L3ZWJcjs.InternalError)("Actor is stopping");
  }
  ...
  async assertCanSubscribe(ctx, eventName) {
    ...
    if (typeof result !== "boolean") {
      throw new (0, _chunkL47L3ZWJcjs.InvalidCanSubscribeResponse)();
    }
    if (!result) {
  ...
  async assertCanPublish(ctx, queueName) {
    ...
    if (typeof result !== "boolean") {
      throw new (0, _chunkL47L3ZWJcjs.InvalidCanPublishResponse)();
    }
    if (!result) {
      throw new (0, _chunkL47L3ZWJcjs.Forbidden)();
    }
  ...
  async executeAction(ctx, actionName, args) {
    ...
    if (!(actionName in actions)) {
      ...
      throw new (0, _chunkL47L3ZWJcjs.ActionNotFound)(actionName);
    }
    ...
    try {
      ...
      spanEnded = true;
      if (isTimeout) {
        throw new (0, _chunkL47L3ZWJcjs.ActionTimedOut)();
      }
      this.#rLog.error({
        msg: "action error",
        actionName,
  ...
  async handleRawRequest(conn, request) {
    ...
    if (!this.#config.onRequest) {
      throw new (0, _chunkL47L3ZWJcjs.RequestHandlerNotDefined)();
    }
    ...
    return await this.runInTraceSpan(
      ...
      async () => {
        ...
        try {
          ...
          if (!response) {
            throw new (0, _chunkL47L3ZWJcjs.InvalidRequestHandlerResponse)();
          }
          return response;
        } catch (error) {
          this.#rLog.error({
  ...
  handleRawWebSocket(conn, websocket, request) {
    ...
    if (!this.#config.onWebSocket) {
      throw new (0, _chunkL47L3ZWJcjs.InternalError)("onWebSocket handler not defined");
    }
  ...
  #initializeTraces() {
    if (_chunk2Q5WS6QHcjs.getRivetExperimentalOtel.call(void 0, )) {
      ...
      this.#traces = _traces.createNoopTraces.call(void 0, );
    }
  }
...
function createActorRouter(config4, actorDriver, getUpgradeWebSocket, isTest) {
  ...
  if (config4.inspector.enabled) {
    ...
    router.get("/inspector/queue", async (c) => {
      ...
      if (authResponse) return authResponse;
      const actor = await actorDriver.loadActor(c.env.actorId);
      const limit = parseInt(_nullishCoalesce(c.req.query("limit"), () => ( "50")), 10);
    ...
    router.get("/inspector/traces", async (c) => {
      ...
      if (authResponse) return authResponse;
      const actor = await actorDriver.loadActor(c.env.actorId);
      const startMs = parseInt(_nullishCoalesce(c.req.query("startMs"), () => ( "0")), 10);
      const endMs = parseInt(
        _nullishCoalesce(c.req.query("endMs"), () => ( String(Date.now()))),
        10
      );
      const limit = parseInt(_nullishCoalesce(c.req.query("limit"), () => ( "1000")), 10);
