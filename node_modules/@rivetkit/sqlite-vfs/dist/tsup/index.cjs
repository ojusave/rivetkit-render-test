...
// src/vfs.ts
var _VFSjs = require('@rivetkit/sqlite/src/VFS.js'); var VFS = _interopRequireWildcard(_VFSjs);





var _sqlite = require('@rivetkit/sqlite');
var _fs = require('fs');
...
var FILE_META_VERSIONED = _vbare.createVersionedDataHandler.call(void 0, {
...
async function loadSqliteRuntime() {
  ...
  return {
    sqlite3: _sqlite.Factory.call(void 0, module),
    module
  };
}
  ...
  async run(sql, params) {
    await this.#sqliteMutex.run(async () => {
      for await (const stmt of this.#sqlite3.statements(this.#handle, sql)) {
        ...
        while (await this.#sqlite3.step(stmt) === _sqlite.SQLITE_ROW) {
        }
      }
    });
  }
    ...
    try {
      ...
      const db = await this.#sqliteMutex.run(
        async () => sqlite3.open_v2(
          fileName,
          _sqlite.SQLITE_OPEN_READWRITE | _sqlite.SQLITE_OPEN_CREATE,
          sqliteSystem.name
        )
      );
      const onClose = () => {
...
var SqliteSystem = (_class = class {
  ...
  #heapDataView;
  #heapDataViewBuffer;
  constructor(sqlite3, module, name) {;_class.prototype.__init.call(this);_class.prototype.__init2.call(this);
    this.name = name;
    this.#sqlite3 = sqlite3;
    this.#module = module;
